...
contract MasterChefCakeERC20 is MasterChefStrategy {
    ...
    function dualTokenDeposit(
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "MasterChefCakeERC20::dualTokenDeposit: Can not deposit zero amount"
        );
        require(
            ((_token0 == address(token0)) &&
                (_token1 == address(token1)) &&
                (_token0 != _token1)),
            "MasterChefCakeERC20::dualTokenDeposit: Invalid token address"
        );
        require(
            slippage >= 1 && slippage < 500,
            "MasterChefCakeERC20::dualTokenDeposit: Invalid slippage"
        );
    ...
    function singleTokenDeposit(
        ...
        require(
            amount > 0,
            "MasterChefCakeERC20::singleTokenDeposit: Can not deposit zero amount"
        );
        require(
            slippage >= 1 && slippage < 500,
            "MasterChefCakeERC20::singleTokenDeposit: Invalid slippage"
        );
        require(
            _token == address(token0) || _token == address(token1),
            "MasterChefCakeERC20::singleTokenDeposit: Invalid token address"
        );
        ...
        nonReentrant
    {
        require(
            _token == address(token0) || _token == address(token1),
            "MasterChefCakeERC20::singleWithdraw: Invalid token address"
        );
    ...
    function reinvest() external onlyEOA nonReentrant {
        ...
        require(
            ...
            "MasterChefCakeERC20::reinvest: MIN_TOKENS_TO_REINVEST"
        );
    ...
    function reinvestOps() external onlyOps nonReentrant {
        ...
        require(
            unclaimedRewards >= MIN_TOKENS_TO_REINVEST,
            "MasterChefCakeERC20::reinvestOps: MIN_TOKENS_TO_REINVEST"
        );
    ...
    function _deposit(uint256 amount, bool check) internal {
        require(
            ...
            "MasterChefCakeERC20::_deposit: deposit failed"
        );
    ...
    function _withdraw(uint256 amount, bool check) internal {
        ...
        if (depositTokenAmount > 0) {
            ...
            require(
                false,
                "MasterChefCakeERC20::_withdraw: withdraw amount can,t be zero"
            );
        }
    }
        ...
        returns (uint256)
    {
        ...
        require(
            amountIn > 0,
            "MasterChefCakeERC20::_convertRewardTokensToDepositTokens: amount too low"
        );
        ...
        path0[1] = WETH;
        path0[2] = path0[0] == address(token0)
            ? address(token1)
            : address(token0);
