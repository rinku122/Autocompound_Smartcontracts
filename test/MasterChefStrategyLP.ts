...
const users = [
  ...
  "0x5a52E96BAcdaBb82fd05763E25335261B270Efcb", //BTCB and BUSD Holder
...
const PID = "36";
const pairAddress = "0xF45cd219aEF8618A92BAa7aD848364a158a24F33";
...
describe("Deposit cake", () => {
  ...
  let cake: CakeToken;
  let strategy: MasterChefStrategyLP;
  let weth: WETH9;
  ...
  beforeEach(async () => {
    ...
    BTCB = await contract.attach(BTCBAddress);
    contract = new ERC20__factory(owner);
    BUSD = await contract.attach(BUSDAddress);
    ...
    await BUSD.connect(user1).transfer(
      owner.address,
      expandTo18Decimals(1050000)
    );

    await BTCB.connect(user1).transfer(owner.address, expandTo18Decimals(55));

    expect(await BUSD.balanceOf(owner.address)).to.be.eq(
      expandTo18Decimals(1050000)
    );
    expect(await BTCB.balanceOf(owner.address)).to.be.eq(
    ...
    //730 BUSD and 5 BTCB left with owner
    await BUSD.approve(router.address, expandTo18Decimals(1049270));
    await BTCB.approve(router.address, expandTo18Decimals(50));

    await router.addLiquidity(
      BTCB.address,
      BUSD.address,
      expandTo18Decimals(50),
      expandTo18Decimals(1049270),
    ...
    token0 = new UniswapV2Pair__factory(owner).attach(token0); //BTCB
    token1 = new UniswapV2Pair__factory(owner).attach(token1); //BUSD
  });
  ...
  it("Reinvest and Withdraw", async () => {
    ...
    await expect(strategy.connect(user2).withdraw("0")).to.be.revertedWith(
      "MasterChefStrategyLP::_withdraw: withdraw amount can,t be zero"
    );
  ...
  it("Deposit single token", async () => {
    await token0.approve(strategy.address, expandTo18Decimals(4));

    await expect(
      strategy.singleTokenDeposit(expandTo18Decimals(0), token0.address, 1)
    ).to.be.revertedWith(
      "MasterChefStrategyLP::singleTokenDeposit: Can not deposit zero amount"
    );
    await expect(
      strategy.singleTokenDeposit(expandTo18Decimals(10), token0.address, 0)
    ).to.be.revertedWith(
      "MasterChefStrategyLP::singleTokenDeposit: Invalid slippage"
    );
    await expect(
      strategy.singleTokenDeposit(expandTo18Decimals(10), token0.address, 501)
    ).to.be.revertedWith(
      "MasterChefStrategyLP::singleTokenDeposit: Invalid slippage"
    );
    await expect(
      strategy.singleTokenDeposit(
        expandTo18Decimals(10),
        "0x2C8171183F8D9B3291876093d1535cf4a10f984D",
        1
      )
    ).to.be.revertedWith(
      "MasterChefStrategyLP::singleTokenDeposit: Invalid token address"
    );
    const reciptTokensBefore = await strategy.balanceOf(user2.address);
    expect(reciptTokensBefore).to.be.eq("0");
    await strategy.singleTokenDeposit(expandTo18Decimals(4), token0.address, 1);
    const reciptTokensAfter = await strategy.balanceOf(owner.address);
    expect(reciptTokensAfter).to.be.above(reciptTokensBefore);

    await expect(
      strategy.singleTokenDeposit(expandTo18Decimals(0), token1.address, 1)
    ).to.be.revertedWith(
      "MasterChefStrategyLP::singleTokenDeposit: Can not deposit zero amount"
    );
  });
  it("Deposit dual tokens", async () => {
    await token0.approve(strategy.address, expandTo18Decimals(4));
    await token1.approve(strategy.address, expandTo18Decimals(500));

    await expect(
      strategy.dualTokenDeposit(
      ...
      "MasterChefStrategyLP::dualTokenDeposit: Can not deposit zero amount"
    );
    await expect(
      strategy.dualTokenDeposit(
        expandTo18Decimals(1),
    ...
    await expect(
      ...
      "MasterChefStrategyLP::dualTokenDeposit: Invalid slippage"
    );
    ...
    await expect(
      ...
      "MasterChefStrategyLP::dualTokenDeposit: Invalid token address"
    );
    await expect(
      strategy.dualTokenDeposit(
        expandTo18Decimals(1),
    ...
    const reciptTokensBefore = await strategy.balanceOf(owner.address);
    expect(reciptTokensBefore).to.be.eq("0");
    await strategy.dualTokenDeposit(
    ...
    expect(await strategy.balanceOf(owner.address)).to.be.above(
      reciptTokensBefore
    );
  });
  it("Single Withdraw", async () => {
    await token1
      .connect(user1)
      .transfer(user2.address, expandTo18Decimals(500));
    await token1
      .connect(user2)
      .approve(strategy.address, expandTo18Decimals(500));
    await strategy
      .connect(user2)
      .singleTokenDeposit(expandTo18Decimals(500), token1.address, 1);
    let reciptTokens = await strategy.balanceOf(user2.address);
    expect(await token1.balanceOf(user2.address)).to.be.eq("0");
    await mineBlocks(ethers.provider, 2550);
    await strategy.connect(user5).reinvestOps();
    await mineBlocks(ethers.provider, 2550);

    await strategy.connect(user5).reinvestOps();
    await mineBlocks(ethers.provider, 2550);
    const totalDepositBefore = await strategy.totalDeposits();
    await strategy.connect(user5).reinvestOps();
    const totalDepositAfter = await strategy.totalDeposits();
    expect(totalDepositAfter).to.be.above(totalDepositBefore);
    reciptTokens = await strategy.balanceOf(user2.address);
    await strategy.connect(user2).singleWithdraw(reciptTokens, token1.address);
