...
async function main() {
  ...
  token = "0x488de67BCF0246ac06ca4d94783817193657c6Cf";
  ...
  try {
    const tokeninstance = tokenInstance(token, owner);
    const sym = await tokeninstance.connect(owner).symbol();
    const _cake = cakeInstance(Cake, owner);
    const _autocompound = autoCompoundInstance(AutoCompound, owner);
    ...
    let pairAddress = await factory.getPair(token, Cake);
    ...
    let contract = new MasterChefCakeERC20__factory(owner);
    masterChefcakeERC20 = await contract.deploy(
      WETH,
      Staking,
      AutoCompound,
      pairAddress,
    ...
    console.log(
      `MasterChefCakeERC20 : ${masterChefcakeERC20.address}, PID : ${PID} `
    );
    console.log(`PairAddress (${sym}-Cake)  : ${pairAddress} `);

    //Change ownership to timelock
  } catch (error) {
    console.log("MasterChefCakeERC20 Error", error);
  }
}
